...
class AdapterLayer(nn.Module):
    def __init__(self, input_dim, reduction_dim=64):
        super(AdapterLayer, self).__init__()
        self.fc1 = nn.Linear(input_dim, reduction_dim)  # 1st fully connected layer
        self.act = nn.ReLU()  # Non-linear activation
        self.fc2 = nn.Linear(reduction_dim, input_dim)  # 2nd fully connected layer
        
    def forward(self, x):
        return x + self.fc2(self.act(self.fc1(x)))  # Residual connection
...
class Qwen2DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        self.attention_adapter = AdapterLayer(config.hidden_size, 8)
        self.fnn_adapter = AdapterLayer(config.hidden_size, 8) #64
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        hidden_states = self.fnn_adapter(hidden_states)
        hidden_states = residual + hidden_states
        return hidden_states
